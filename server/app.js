...
app.get('/api/status', (req, res) => {
  ...
  res.json({ code: 0, data: {
    ...
    appVersion: settings.appVersion || '4.0.0',
    author: '一屿'
  }});
});
...
app.post('/api/auth/register', (req, res) => {
  ...
  res.json({
    code: 0, message: '注册成功',
    data: { token, user: {
      id: newUser.id, username, phone, qq,
      avatar: '', level: newUser.level, isVip: false
    }}
  });
});
...
app.post('/api/auth/login', (req, res) => {
  ...
  res.json({
    ...
    data: {
      token,
      user: {
        id: user.id, username: user.username, phone: user.phone, qq: user.qq,
        avatar: user.avatar || '', level: user.level, isVip: !!user.isVip,
        vipExpireAt: user.vipExpireAt, isAdmin: user.isAdmin,
        lastEditProfileAt: user.lastEditProfileAt
      }
    }
  });
});
...
app.get('/api/auth/me', authMiddleware, (req, res) => {
  res.json({
    code: 0,
    data: {
      id: req.user.id, username: req.user.username, phone: req.user.phone, qq: req.user.qq,
      avatar: req.user.avatar || '', level: req.user.level, isVip: !!req.user.isVip,
      vipExpireAt: req.user.vipExpireAt, isAdmin: req.user.isAdmin,
      lastEditProfileAt: req.user.lastEditProfileAt
    }
  });
});
...
app.get('/api/user/history', authMiddleware, (req, res) => {
  res.json({ code: 0, data: req.user.history || [] });
});

app.post('/api/user/history', authMiddleware, (req, res) => {
  const { vodId, sourceKey, title, cover, episode } = req.body;
  ...
  user.history.unshift({
    ...
    episode: episode || '', watchedAt: new Date().toISOString()
  });
  if (user.history.length > 200) user.history.length = 200;
  db.writeJSON('users', users);
  res.json({ code: 0, message: '已记录观看历史' });
});

app.delete('/api/user/history', authMiddleware, (req, res) => {
  const { vodId, sourceKey } = req.body;
  const users = db.readJSON('users', []);
  const user = users.find(u => u.id === req.user.id);
  if (!user) return res.json({ code: 1, message: '用户不存在' });
  if (vodId && sourceKey) {
...
app.get('/api/user/favorites', authMiddleware, (req, res) => {
  res.json({ code: 0, data: req.user.favorites || [] });
});
...
app.put('/api/admin/users/:id', adminMiddleware, (req, res) => {
  const { id } = req.params;
  const { phone, qq, level, isVip, vipExpireAt, password, isBanned } = req.body;
  const users = db.readJSON('users', []);
  ...
  if (vipExpireAt !== undefined) user.vipExpireAt = vipExpireAt;
  if (isBanned !== undefined) user.isBanned = isBanned;
  if (password) user.password = db.hashPassword(password);
  db.writeJSON('users', users);
  logOperation(`修改用户 ${user.username}`, req.user.username, getIP(req));

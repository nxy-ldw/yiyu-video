...
/** 封装 DELETE 请求 */
function apiDelete(path, callback, onError) {
  fetch(getApiUrl(path), {
    method: 'DELETE',
...
function renderMineUserCard() {
  ...
  if (nameEl) {
    var vipIcon = u.isVip ? ' <span style="display:inline-block;margin-left:6px;padding:2px 8px;border-radius:8px;background:linear-gradient(135deg,#f6d365,#fda085);color:#fff;font-size:11px;font-weight:700;vertical-align:middle;">👑 VIP</span>' : '';
...
function toggleFav() {
  ...
  var isFav = document.getElementById('detail-fav-btn').classList.contains('favorited');
  if (isFav) {
    apiDelete('/user/favorites?sourceKey=' + encodeURIComponent(sourceKey) + '&vodId=' + encodeURIComponent(id), function (res) {
      if (!res || res.code === 0) {
